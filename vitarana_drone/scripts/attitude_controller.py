...
class Edrone():
    ...
    def calc_pid(self):
	self.roll = self.Kp[0]*self.error[0] + self.Ki[0]*self.error_sum[0] + self.Kd[0]*(self.error[0]-self.prev_error[0])
	self.pitch = self.Kp[1]*self.error[1] + self.Ki[1]*self.error_sum[1] + self.Kd[1]*(self.error[1]-self.prev_error[1])
	self.yaw = self.Kp[2]*self.error[2] + self.Ki[2]*self.error_sum[2] + self.Kd[2]*(self.error[2]-self.prev_error[2])
	...
	self.pwm_cmd.prop1 = self.throttle + self.roll + self.pitch #+ self.yaw
	self.pwm_cmd.prop2 = self.throttle - self.roll + self.pitch #- self.yaw
	self.pwm_cmd.prop3 = self.throttle + self.roll - self.pitch #- self.yaw
	self.pwm_cmd.prop4 = self.throttle - self.roll - self.pitch #+ self.yaw
